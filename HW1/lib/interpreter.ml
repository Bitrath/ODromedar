...
(*
  ...
  *)
  ...
  | Let (x, eRhs, letBody) -> 
     let xVal, t1 = eval eRhs env t sec_lev in (* xVal: evT * bool != xVal, t1: evT, bool *)
      let letEnv = (x, xVal, t1)::env in
        eval letBody letEnv t1 sec_lev 
  | Prim (ope, e1, e2) -> (
      let v1, t1 = eval e1 env t sec_lev in
        let v2, t2 = eval e2 env t  sec_lev in
  ...
  | If (cond, e2, e3) -> (
      let v1, t1 = eval cond env t sec_lev in
        match v1 with
...
(* --- [ >:( ] --- *)
  | Call (f, param) -> ( (* Call -> chiamata a funzione f *)
      ...
      let fClosure, f_t = eval f env t sec_lev in 
        ...
        match fClosure, f_t  with (* fClosure -> funzione insieme all'ambiente in 
          ...
          (* CLOSURE: la funzione viene eseguita solo se il valore di f_t è UNTAINTED *) 
            (* xVal is evaluated in the current stack *)
              let xVal, t1 = eval param env t sec_lev in (* param (che rappresenta gli argomenti 
                ...
                (* costruzione di un nuovo ambiente fBodyEnv estendendo l'ambiente 
                  ...
                  let f_res, t_res = eval f_body fBodyEnv t sec_lev in 
