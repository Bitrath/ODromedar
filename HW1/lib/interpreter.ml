...
let rec eval (e: exp) (env: evT env) (stack : pstack) (t : bool) (block: block_level): evT * bool = 
  ...
  | Let (x, conf_lev, eRhs, letBody) ->(
      ...
      | _ ->
            let xVal, t1 = eval  eRhs env stack t block in
            (* xVal: evT * bool != xVal, t1: evT, bool *)
  ...
  | Call (f, param) -> (
      let fClosure, f_t = eval f env stack t block in
         match fClosure, f_t  with
          ...
          (*CLOSURE: la funzione viene valutata solo se il valore di f_t è UNTAINED ,cioè FALSE*)

          | Closure (f_param, f_body, secSet, fDeclEnv),  false  ->
              (* xVal is evaluated in the current stack *)
              ...
              let f_res, t_res = eval f_body fBodyEnv fBodyStack t block

              (*NON HO PIÙ BISOGNO DI OR TRA I TAINTED VALUES*)
               in (f_res, t_res)
