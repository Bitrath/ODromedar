...
type secAction = Grant of pdomain
...
del pdomain, richiamando domainInspection. PASSA IN RASSEGNA tutti GLI ELEMENTI DI PDOMAIN FINCHÈ UNA ~EGUAGLIANZA NON OCCORRE *)
let rec domainInspection (set: pdomain) (request: permission) = 
  match set with
    | [] -> 0 
    | p::s -> if allows p request   
        then 1 else domainInspection s request
